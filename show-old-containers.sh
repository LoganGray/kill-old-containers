...
function show_old_containers(){
    ...
    IFS=$INITIAL_IFS
    printf "showing containers older than ${1} seconds:\n\n"
    CONT_COUNT=${#OUTPUT[@]}
    ...
    do
        ...
        then
            ...
            SHOW_CMD="docker inspect ${CONT_ID}"
    ...
    if [[ $NUM_SHOWED -gt 0 ]]; then
        printf "\n  $NUM_SHOWED containers."
    fi
}
...
show_old_containers "$TIME" "$SEARCH"
