...
function restart_old_containers(){
    ...
    IFS=$INITIAL_IFS
    printf "Restarting containers older than ${1} seconds:\n\n"
    CONT_COUNT=${#OUTPUT[@]}
    ...
    do
        ...
        then
            ...
            restart_CMD="docker restart ${CONT_ID}"
            # invoke docker restart command in the background to prevent
            # possible freezing
            echo "${restart_CMD}"
            eval "${restart_CMD}" &>/dev/null & disown
            let "NUM_restartED+=1"
        fi
    done
    if [[ $NUM_restartED -gt 0 ]]; then
        printf "\nRestarted $NUM_restartED containers."
    fi
}
...
restart_old_containers "$TIME" "$SEARCH"
